...
const validationSchema = yup.object().shape({
    ...
    // storrelse: yup.mixed<PartnerStorrelse>().label('Størrelse på partneren').required(),
});
...
export const AddPartnerForm: React.FC<Props> = ({ afterSubmit }) => {
    const formMethods = useForm<{ navn: string }>({ resolver: yupResolver(validationSchema) });
    ...
    return (
        <FormProvider {...formMethods}>
            <form onSubmit={handleNewPartnerSubmission}>
                ...
                {/*<Box backgroundColor="error">{formState.errors.toString()}</Box>*/}
